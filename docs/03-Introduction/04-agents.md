...
Agents are responsible for gathering and reporting information about the endpoints to OpenUEM. Agents are also responsible for: 

- Downloading and installing apps using WinGet, Flatpak or Brew 
- Applying configuration profiles 
- Running an SFTP server, if enabled, for file browsing 
- Running a VNC proxy, if enabled, to encrypt VNC traffic between the user’s browser and the remote endpoint 
