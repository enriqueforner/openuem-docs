...
- Agent Worker, receives information from the agents and stores it in the PostgreSQL database.
- Notification Worker will send email messages to users (confirm email address, attach certificates to log in) after an administrator adds an SMTP configuration.
